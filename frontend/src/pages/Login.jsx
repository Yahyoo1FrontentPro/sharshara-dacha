    ...
    <div style={{ maxWidth: '400px', margin: '4rem auto' }}>
      <div className="card">
        <div className="card-body">
          <h2 style={{ textAlign: 'center', color: 'var(--color-primary)' }}>{t('login_title')}</h2>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn btn-primary btn-block">{t('login_btn')}</button>
          </form>
          <div style={{ marginTop: '1rem', textAlign: 'center', color: 'var(--color-text-muted)' }}>
            {t('no_account')} <Link to="/register" style={{ color: 'var(--color-primary)' }}>{t('register_btn')}</Link>
          </div>
        </div>
      </div>
    </div>
